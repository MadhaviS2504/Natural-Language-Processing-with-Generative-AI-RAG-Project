...
from src.config import CHROMA_PERSIST_DIR, PDF_PATH, SAMPLE_QUERIES, TOP_K_RETRIEVAL
from src.ingestion import ingest_pipeline
from src.retriever import build_vector_store, get_retriever, load_vector_store, retrieve_context, get_vector_store
...
def run_all_approaches(query: str, retriever=None, k: int = TOP_K_RETRIEVAL) -> dict:
    """
    Run a query through all three generation approaches.

    Args:
        query:     The medical question to answer.
        retriever: Initialized LangChain retriever. If None, creates one.
...
def run_rag_only(query: str, k: int = TOP_K_RETRIEVAL) -> dict:
    """
    Run only the RAG approach (for API usage).
    ...
    Returns:
        Dictionary with keys: 'query', 'answer', 'context', 'sources'.
    """
